...
all_gossips.each do |potin|
    all_goosip_tags << GoosipTag.create(
        potin: potin,
        tag: all_tags.sample
    )
end
4.times do
    ...
    recipent = all_users.reject{ |i| i == sender}.sample
    all_private_messages << PrivateMessage.create(
...
20.times do
    random = rand(1..2)
    if random == 1
        all_likes_comment << LikeComment.create(
            user: all_users.sample, 
            comment: all_comments.sample)
    else
        all_likes_gossip << LikeGossip.create(
            user:all_users.sample,
            potin:all_gossips.sample
        )
    end
